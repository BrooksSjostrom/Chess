...
public class SimpleEvaluator {
  ...
  public int eval(Board board, ChessColor turn) {
    // Get current player
    ChessColor player = turn;
    int playerValue = 0;
    
    // Opponent is opposite of player
    ChessColor opponent = turn.opposite();
    int opponentValue = 0;
  
    List<Piece> pieces = board.getPiecesOfColor(player);
    playerValue = getValueOfPieces(pieces, player);
    
    pieces = board.getPiecesOfColor(opponent);
...
//    }
  ...
  private int getValueOfPieces(List<Piece> pieces, ChessColor color) {
    
    // Determine pawn array to use
    int[][] pawnpos = color == ChessColor.WHITE ? pawnposWhite : pawnposBlack;
    
    int value = 0;
    for(Piece piece : pieces) {
      switch(piece.getType()) {
        case PAWN:
          value += pawnval + pawnpos[piece.position.y][piece.position.x];
          break;        
        case BISHOP:
          value += bishopval + bishoppos[piece.position.y][piece.position.x];
          break;        
        case KNIGHT:
          value += knightval + knightpos[piece.position.y][piece.position.x];
          break;
        case ROOK:
          value += rookval;                             
          break;
